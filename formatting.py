...
def oasst_format(data):
    formatted_data = []
    for entry in data:
        new_entry = {}
        system_instruction = entry["messages"][0]["content"].split('## SYSTEM INSTRUCTION:')
        user_instruction = system_instruction[1].split('## USER INSTRUCTION:')
        new_entry["prompt"] = system_instruction[0].strip() + " " + user_instruction[0].strip()
        new_entry["response"] = entry["messages"][1]["content"]
        formatted_data.append(new_entry)
    return formatted_data
...
def main(args):
    ...
    elif args.format == "Oasst":
        formatted_data = oasst_format(data)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-format', required=True, choices=["ShareGPT", "Alpaca", "Oasst"], help='Format to conform to')
    ...
    if args.format == "ShareGPT":
        ...
        if args.input_json and not args.output_json:
            args.output_json = re.sub(r'_([^_]*)$', '_oasst', args.input_json)
