...
def process_entry_thread(i, entry, args, output_data_len):
    ...
    with print_lock:
        print(f"threaded_askgpt: [Thread-{thread_id}] messages_id {entry['messages_id']} PROCESSING ({i+1}/{output_data_len})")
    ...
    while entry.get('messages_complete') != True:
        try:
            ...
            with print_lock:
                print(f"threaded_askgpt: [Thread-{thread_id}] messages_id {entry['messages_id']} COMPLETE ")
        
        except Exception as e:
            print(f"threaded_askgpt: [Thread-{thread_id}] messages_id {entry['messages_id']} ERROR: {e} (Retrying in 5-15 seconds...)")
